...
function route(array $config): void
{
    ...
    if ($method === 'POST' && $path === '/api/taxis/registros') {
        ...
        sync_taxista_badges($db, isset($data['catalogId']) ? (int)$data['catalogId'] : null, clean($data['driverName'] ?? ''), $badgeIds, true, $branchCode, clean($data['sellerKey'] ?? ''), clean($data['sellerName'] ?? ''));
    ...
    if ($method === 'PUT' && preg_match('#^/api/taxis/registros/([^/]+)$#', $path, $m)) {
        ...
        sync_taxista_badges($db, isset($data['catalogId']) ? (int)$data['catalogId'] : (isset($current['catalogId']) ? (int)$current['catalogId'] : null), clean($data['driverName'] ?? $current['driverName']), $badgeIds, true, $branchCode, clean($data['sellerKey'] ?? $current['sellerKey']), clean($data['sellerName'] ?? $current['sellerName']));

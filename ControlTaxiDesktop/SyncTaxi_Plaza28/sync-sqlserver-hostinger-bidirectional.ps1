...
function Normalize-BadgeStatusForHostinger([string]$Status) {
    $statusText = (Text $Status).Trim().ToUpperInvariant()
    if ($statusText -eq "R" -or $statusText -eq "S") { return "Disponible" }
    if ($statusText -eq "A") { return "Asignado" }
    return ""
}
...
function Ensure-AppTables($Connection) {
    $quickSchema = $Connection.CreateCommand()
    $quickSchema.CommandTimeout = 30
    $quickSchema.CommandText = @"
IF OBJECT_ID('dbo.AppMovilRegistro', 'U') IS NOT NULL
   AND OBJECT_ID('dbo.AppMovilRegistroGafetes', 'U') IS NOT NULL
   AND OBJECT_ID('dbo.dejadas', 'U') IS NOT NULL
   AND OBJECT_ID('dbo.gafete', 'U') IS NOT NULL
BEGIN
    ...
    IF COL_LENGTH('dbo.AppMovilRegistro', 'no_show_count') IS NULL ALTER TABLE dbo.AppMovilRegistro ADD no_show_count INT NOT NULL DEFAULT 0;
    IF COL_LENGTH('dbo.dejadas', 'adl') IS NULL ALTER TABLE dbo.dejadas ADD adl INT NULL;
    IF COL_LENGTH('dbo.dejadas', 'men') IS NULL ALTER TABLE dbo.dejadas ADD men INT NULL;
    IF COL_LENGTH('dbo.dejadas', 'inf') IS NULL ALTER TABLE dbo.dejadas ADD inf INT NULL;
    IF COL_LENGTH('dbo.dejadas', 'nacionalidad') IS NULL ALTER TABLE dbo.dejadas ADD nacionalidad NVARCHAR(120) NULL;
    SELECT 1;
END
ELSE
BEGIN
...
END;
"@
    if ([int]$quickSchema.ExecuteScalar() -eq 1) {
        return
    }
...
BEGIN
    CREATE TABLE dbo.AppMovilRegistro (
        ...
        seller_key NVARCHAR(50) NOT NULL DEFAULT '',
        seller_name NVARCHAR(150) NOT NULL DEFAULT '',
...
IF 1 = 0 AND OBJECT_ID('dbo.dejadas', 'U') IS NOT NULL
...
IF 1 = 0 AND OBJECT_ID('dbo.gafete', 'U') IS NOT NULL
...
BEGIN
    ...
    SET folio_app_original = @folioOriginal,
        ...
        payout_user = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutUser ELSE payout_user END,
        payout_ticket = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutTicket ELSE payout_ticket END,
        estado_pago_dejada = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutStatus ELSE estado_pago_dejada END,
        fecha_pago_dejada = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutDate ELSE fecha_pago_dejada END,
        usuario_pago_dejada = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutUser ELSE usuario_pago_dejada END,
        ticket_pago_dejada = CASE WHEN UPPER(@payoutStatus) IN ('PAGADO','PAGADA','PAID') THEN @payoutTicket ELSE ticket_pago_dejada END
    WHERE folio_app = @folioControl
       OR folio_app_original = @folioOriginal;
END
ELSE
BEGIN
    ...
    (
        ...
        vendedor_nombre, vendedor_clave, seller_key, seller_name, adult_count, youth_count, minor_count, no_show_count,
        telefono_taxista, telefono_contacto, nacionalidad, placas, modelo_vehiculo,
    ...
    (
        ...
        @driverName, @sellerKey, @sellerKey, @sellerName, @adultCount, @youthCount, @minorCount, @noShowCount,
        @driverPhone, @contactPhone, @nationality, @plate, @vehicleModel,
...
"@
    ...
    Add-Parameter $command "@sellerKey" (Text-Max $Record.sellerKey 50)
    Add-Parameter $command "@sellerName" (Text-Max $Record.sellerName 150)
    Add-Parameter $command "@adultCount" (Int-Value $Record.adultCount)
    Add-Parameter $command "@youthCount" (Int-Value $Record.youthCount)
    Add-Parameter $command "@minorCount" (Int-Value $Record.minorCount)
    Add-Parameter $command "@noShowCount" (Int-Value $Record.noShowCount)
...
DECLARE @computedPax INT = CASE WHEN @passengerCount > 0 THEN @passengerCount ELSE @adultCount + @youthCount + @minorCount END;

IF NOT EXISTS (
    SELECT 1
    FROM dbo.dejadas WITH (UPDLOCK, HOLDLOCK)
    WHERE folioregistrostr = @folioControl
      AND gafete = @badgeId
      AND fecha >= CONVERT(DATETIME, CONVERT(DATE, @recordDate))
      AND fecha < DATEADD(DAY, 1, CONVERT(DATETIME, CONVERT(DATE, @recordDate)))
)
BEGIN
    INSERT INTO dbo.dejadas (
        ...
        @folioControl,COALESCE(@folioBigint, 0),@folioControl,LEFT(@unitNumber, 20),@computedPax,LEFT(@hotel, 100),LEFT(@sellerName, 100),
        LEFT(@serviceType, 10),LEFT(@driverPhone, 12),@horaTexto,@horaTexto,0,0,0,
        @cashAmount,@cardAmount,0,LEFT(@badgeId, 10),@catalogId,@adultCount,@youthCount,@minorCount,LEFT(@nationality, 120)
    );
END
ELSE
BEGIN
    UPDATE dbo.dejadas
    SET nombrevendedor = LEFT(@sellerName, 100),
        pax = @computedPax,
        adl = @adultCount,
        men = @youthCount,
        inf = @minorCount,
...
"@
        ...
        Add-Parameter $command "@sellerName" (Text-Max $Record.sellerName 100)
...
BEGIN
    SELECT TOP (0)
        CONVERT(NVARCHAR(400), '') AS BadgeValue,
...
SELECT TOP (300)
    CONVERT(NVARCHAR(400), gafete) AS BadgeValue,
    UPPER(LTRIM(RTRIM(COALESCE(venta, '')))) AS Status,
    COALESCE(hora, fecha, GETDATE()) AS CreatedAt
FROM dbo.gafete
WHERE gafete IS NOT NULL
  AND UPPER(LTRIM(RTRIM(COALESCE(venta, '')))) IN ('A', 'R', 'S')
  AND COALESCE(hora, fecha, GETDATE()) >= DATEADD(DAY, -14, GETDATE())
ORDER BY COALESCE(hora, fecha, GETDATE()) DESC;
"@
        ...
        foreach ($row in $table.Rows) {
            $badges = @(Split-Badges $row["BadgeValue"])
            $status = (Text $row["Status"]).ToUpperInvariant()
            $createdAt = [datetime]$row["CreatedAt"]
            foreach ($badge in $badges) {
                if ([string]::IsNullOrWhiteSpace($badge)) { continue }
                $badgeKey = $badge.Trim()
                if (-not $latestByBadge.ContainsKey($badgeKey) -or $createdAt -gt $latestByBadge[$badgeKey].CreatedAt) {
                    $latestByBadge[$badgeKey] = [pscustomobject]@{
                        BadgeId = $badgeKey
                        Status = $status
                        CreatedAt = $createdAt
                    }
        ...
        $items = @()
        foreach ($entry in $latestByBadge.GetEnumerator()) {
            $row = $entry.Value
            if ($row.Status -ne "R" -and $row.Status -ne "S") { continue }
            $hostingerStatus = Normalize-BadgeStatusForHostinger $row.Status
            if ([string]::IsNullOrWhiteSpace($hostingerStatus)) { continue }

            $items += [ordered]@{
                badgeId = $row.BadgeId
                barcode = $row.BadgeId
                status = $hostingerStatus
                cycle = 1
                taxistaId = $null
                taxistaName = ""
                createdAt = $row.CreatedAt.ToString("s")
            }
        }
        ...
        Write-Host "Subiendo estados R/S de gafetes a Hostinger como Disponible: $($items.Count)"
        $payload = [ordered]@{
            mkt2_gafetes = $items
        }

...
function route(array $config): void
{
    ...
    if ($method === 'POST' && $path === '/api/taxis/registros') {
        ...
        save_trip_record($db, [
            ...
            'sellerKey' => clean($data['sellerKey'] ?? ''),
            'sellerName' => clean($data['sellerName'] ?? ''),
        ...
        sync_taxista_badges($db, isset($data['catalogId']) ? (int)$data['catalogId'] : null, clean($data['driverName'] ?? ''), $badgeIds, true, $branchCode);
        json_response(['recordId' => $recordId, 'recordDate' => date('c')]);
    }
    if ($method === 'PUT' && preg_match('#^/api/taxis/registros/([^/]+)$#', $path, $m)) {
        ...
        save_trip_record($db, [
            ...
            'sellerKey' => clean($data['sellerKey'] ?? $current['sellerKey']),
            'sellerName' => clean($data['sellerName'] ?? $current['sellerName']),
        ...
        sync_taxista_badges($db, isset($data['catalogId']) ? (int)$data['catalogId'] : (isset($current['catalogId']) ? (int)$current['catalogId'] : null), clean($data['driverName'] ?? $current['driverName']), $badgeIds, true, $branchCode);
    ...
    if ($method === 'POST' && preg_match('#^/api/taxis/registros/([^/]+)/pagar$#', $path, $m)) {
        ...
        json_response(row($db, "SELECT recordId,catalogId,badgeId,driverName,driverPhone,contactPhone,nationality,plate,vehicleModel,unitNumber,hotel,origin,site,destination,sellerKey,sellerName,adultCount,youthCount,minorCount,noShowCount,passengerCount,serviceType,tripCost,paymentMethod,notes,recordDate,assignedBranch,assignedBranchCode,assignedBranchName,payoutStatus,payoutDate,payoutUser,payoutTicket FROM mkt2_trip_records WHERE recordId=?", [$recordId]));
    }
...
function sync_taxista_badges(PDO $db, ?int $taxistaId, string $taxistaName, mixed $badgeValue, bool $active, string $branchCode = ''): void
{

...
public sealed class CascoApplyNewService
{
    private const int MaxBatchSize = 10;

    public async Task<CascoApplyNewResult> ApplyNewAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var pendingRecords = new List<(CascoTripRecord Record, string ProposedFolio)>();
        var omittedByExisting = 0;
        foreach (var candidate in orderedCandidates)
        {
            ...
            var proposedFolio = await ResolveLocalFolioAsync(connection, candidate.RecordId, cancellationToken);
            ...
            pendingRecords.Add((candidate, proposedFolio));
        ...
        Console.WriteLine($"Registros remotos encontrados: {orderedCandidates.Count}");
        Console.WriteLine($"Registros nuevos para insertar: {pendingRecords.Count}");
        if (pendingRecords.Count == 0)
        {
            Console.WriteLine("No hay registros nuevos para insertar.");
            return new CascoApplyNewResult
            {
                Applied = false,
                Confirmed = false,
                RecordsFound = orderedCandidates.Count,
                RecordsToInsert = 0,
                InsertedCount = 0,
                OmittedCount = omittedByExisting,
                ErrorCount = 0,
                GeneratedFolios = Array.Empty<string>()
            };
        }

        Console.WriteLine("\nrecordId | folio propuesto | conductor | fecha | total");
        foreach (var (record, proposedFolio) in pendingRecords)
        {
            Console.WriteLine($"{record.RecordId} | {proposedFolio} | {record.DriverName} | {record.RecordDate:yyyy-MM-dd HH:mm} | {record.TripCost?.ToString(CultureInfo.InvariantCulture) ?? "0"}");
        }

        Console.WriteLine("\nEscriba INSERTAR TODOS para continuar.");
        var confirmation = Console.ReadLine();
        if (!string.Equals(confirmation, "INSERTAR TODOS", StringComparison.Ordinal))
        {
            ...
            return new CascoApplyNewResult
            {
                ...
                Confirmed = false,
                RecordsFound = orderedCandidates.Count,
                RecordsToInsert = pendingRecords.Count,
                InsertedCount = 0,
                OmittedCount = omittedByExisting,
                ErrorCount = 0,
                GeneratedFolios = pendingRecords.Select(x => x.ProposedFolio).ToArray()
            };
        }
        ...
        var inserted = 0;
        var errors = 0;
        var generatedFolios = new List<string>();
        foreach (var (record, proposedFolio) in pendingRecords)
        {
            var localCheck = await ExistsLocalRecordAsync(connection, record.RecordId, record.Site, cancellationToken);
            if (localCheck)
            {
                omittedByExisting++;
                continue;
            ...
            if (!string.Equals(record.Site, "Casco Viejo", StringComparison.OrdinalIgnoreCase))
            {
                logger.Log($"apply-new: sitio no válido para {record.RecordId}: {record.Site}");
                errors++;
                continue;
            }

            var folioCheck = await ExistsLocalFolioAsync(connection, proposedFolio, cancellationToken);
            if (folioCheck)
            {
                logger.Log($"apply-new: folio ya existente para {record.RecordId}: {proposedFolio}");
                errors++;
                continue;
            }
            ...
            try
            {
                ...
                try
                {
                    var alreadyExists = await ExistsLocalRecordAsync(connection, record.RecordId, record.Site, transaction, cancellationToken);
                    if (alreadyExists)
                    {
                        omittedByExisting++;
                    ...
                    var folioStillTaken = await ExistsLocalFolioAsync(connection, proposedFolio, transaction, cancellationToken);
                    if (folioStillTaken)
                    {
                        logger.Log($"apply-new: folio tomado durante la validación para {record.RecordId}: {proposedFolio}");
                        await transaction.RollbackAsync(cancellationToken);
                        errors++;
                        continue;
                    }

                    var insertValues = BuildInsertValues(record, columnMetadata, proposedFolio);
                    var requiredColumns = GetRequiredColumns(columnMetadata);
                    ...
                    insertCommand.CommandText = BuildInsertStatement(insertValues, includeParameters: true);
                    foreach (var parameter in CreateParameters(insertValues))
...
WHERE folio_app = @folio_app AND folio_app_original = @folio_app_original;";
                    ...
                    validationCommand.Parameters.AddWithValue("@folio_app_original", record.RecordId ?? string.Empty);
                    ...
                    inserted++;
                    generatedFolios.Add(proposedFolio);
                    logger.Log($"apply-new: insertado {record.RecordId} con folio {proposedFolio}");
                }
                catch
                {
                    await transaction.RollbackAsync(cancellationToken);
            ...
            {
                errors++;
                logger.Log($"apply-new: error al insertar {record.RecordId}: {ex.Message}");
            }
        }

        Console.WriteLine($"\nResumen apply-new");
        Console.WriteLine($"Registros remotos evaluados: {orderedCandidates.Count}");
        Console.WriteLine($"Insertados: {inserted}");
        Console.WriteLine($"Omitidos por existentes: {omittedByExisting}");
        Console.WriteLine($"Errores: {errors}");
        Console.WriteLine($"Folios generados: {string.Join(", ", generatedFolios)}");

        return new CascoApplyNewResult
        {
            Applied = inserted > 0,
            Confirmed = inserted > 0,
            RecordsFound = orderedCandidates.Count,
            RecordsToInsert = pendingRecords.Count,
            InsertedCount = inserted,
            OmittedCount = omittedByExisting,
            ErrorCount = errors,
            GeneratedFolios = generatedFolios.ToArray()
        };
    }
    ...
    private static async Task<bool> ExistsLocalRecordAsync(SqlConnection connection, string? recordId, string? site, CancellationToken cancellationToken)
        => await ExistsLocalRecordAsync(connection, recordId, site, null, cancellationToken);
...
WHERE folio_app_original = @recordId AND sitio = @site;";
    ...
    private static async Task<bool> ExistsLocalFolioAsync(SqlConnection connection, string folio, CancellationToken cancellationToken)
        => await ExistsLocalFolioAsync(connection, folio, null, cancellationToken);

    private static async Task<bool> ExistsLocalFolioAsync(SqlConnection connection, string folio, SqlTransaction? transaction, CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = @"
...
ORDER BY ORDINAL_POSITION;";
    ...
    private static HashSet<string> GetRequiredColumns(IReadOnlyList<ColumnMetadata> columnMetadata)
    {
    ...
    private static IReadOnlyList<SqlParameter> CreateParameters(IDictionary<string, object?> values)
        => values.Select(x => CreateParameter(x.Key, x.Value)).ToList();
...
public sealed record CascoApplyNewResult
{
    ...
    public IReadOnlyList<string> GeneratedFolios { get; init; } = Array.Empty<string>();
}

public sealed record ColumnMetadata(string ColumnName, string DataType, string IsNullable, string? ColumnDefault);

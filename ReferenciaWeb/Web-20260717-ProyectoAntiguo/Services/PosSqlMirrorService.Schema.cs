...
BEGIN
    ...
    IF COL_LENGTH('{PosTable("RelacionTicketTaxista").Replace("[", string.Empty).Replace("]", string.Empty)}', 'SeFueron') IS NULL
        ALTER TABLE {PosTable("RelacionTicketTaxista")} ADD SeFueron INT NULL;
  END;";
            await command.ExecuteNonQueryAsync();
        }
    }

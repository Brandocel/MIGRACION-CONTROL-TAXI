...
ORDER BY COALESCE(m.fecha, o.fecha) DESC, m.folioperacion DESC",
        ...
        private async Task ApplyAuthoritativeTransportRulesAsync(List<PosComisionRowViewModel> comisiones)
        {
            ...
            foreach (var item in comisiones)
            {
                ...
                var salmoran = nombre.Contains("SALMORAN", StringComparison.OrdinalIgnoreCase)
                    || string.Equals(transporte.Tipo, "SALMORAN", StringComparison.OrdinalIgnoreCase);
                var porcentaje = majestic ? 8m : salmoran ? 20m : ToDecimal(transporte.Comision);
...
(
    ...
    INNER JOIN (VALUES {values}) v(Ticket)
        ...
        private async Task ApplyTicketPaymentBreakdownRulesAsync(List<PosComisionRowViewModel> comisiones)
        {
            ...
            {
                var ticket = Normalize(item.Ticket);
                if (ticket == null || !paymentsByTicket.TryGetValue(ticket, out var payments) || payments.Total <= 0m)
                    continue;

                item.FormaPago = payments.Description;
                if (payments.Card <= 0m && payments.Amex <= 0m)
                    continue;
                ...
                item.DescuentoAplicado = Math.Round(descuentoEfectivo, 2);
        ...
        private async Task<Dictionary<string, TicketPaymentBreakdown>> GetTicketPaymentBreakdownsAsync(IEnumerable<string?> tickets)
        {
            ...
            var parameters = validTickets
                ...
                .ToList();
            var values = string.Join(", ", parameters.Select(x => $"({x})"));
            var compuadmoPlazaDatabase = PlazaDatabaseName(_compuadmoStoreDatabaseName);
            var joyeriaPlazaDatabase = PlazaDatabaseName(_joyeriaStoreDatabaseName);
            command.CommandText = $@"
SELECT Ticket, PaymentName, Total
FROM
(
    SELECT
        UPPER(LTRIM(RTRIM(CONVERT(nvarchar(80), p.folio_factura)))) AS Ticket,
        LTRIM(RTRIM(COALESCE(m.Nombre, ''))) AS PaymentName,
        CAST(COALESCE(p.total, 0) AS decimal(18,2)) AS Total
    FROM {QuoteSqlIdentifier(compuadmoPlazaDatabase)}.{QuoteSqlIdentifier(_storeSchemaName)}.[pagosM] p
    INNER JOIN {QuoteSqlIdentifier(compuadmoPlazaDatabase)}.{QuoteSqlIdentifier(_storeSchemaName)}.[monedas] m
        ON m.moneda = p.moneda
    INNER JOIN (VALUES {values}) v(Ticket)
        ON UPPER(LTRIM(RTRIM(CONVERT(nvarchar(80), p.folio_factura)))) = v.Ticket
    ...
    SELECT
        ...
        LTRIM(RTRIM(COALESCE(m.Nombre, ''))) AS PaymentName,
        CAST(COALESCE(p.total, 0) AS decimal(18,2)) AS Total
    FROM {QuoteSqlIdentifier(joyeriaPlazaDatabase)}.{QuoteSqlIdentifier(_storeSchemaName)}.[pagosM] p
    INNER JOIN {QuoteSqlIdentifier(joyeriaPlazaDatabase)}.{QuoteSqlIdentifier(_storeSchemaName)}.[Monedas] m
        ON m.Moneda = p.moneda
    INNER JOIN (VALUES {values}) v(Ticket)
        ON UPPER(LTRIM(RTRIM(CONVERT(nvarchar(80), p.folio_factura)))) = v.Ticket
) Payments
...
WHERE Importe > 0;";
        ...
        private readonly record struct TicketPaymentBreakdown(
            decimal NonCard,
            decimal Card,
            decimal Amex,
            string Description)
